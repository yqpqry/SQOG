# -*- coding: utf-8 -*-
#Source Code of KL_SQOG (BRAC+SBO), 20250208
...
class TrainArgs:
    # Experiment
    exp_name: str = "KL_SQOG"
    env_id: str = "pen"
    gym_id: str = env_id + '-' +'human-v1'
    seed: int = 72
    ...
    log_freq: int = 1000
    beta: float = 2.5
    alpha: float = 25 # 300,200不如150，尝试更小的, 更小的也不好  
    policy_noise_2: float = 0.6
    noise_clip_2: float = 0.5
    bc_train: int = 100000
    ...
    def __post_init__(self):
        #self.exp_name = f"{self.exp_name}__{self.gym_id}__a:{self.alpha}__b:{self.beta}"
        self.exp_name = f"{self.exp_name}__{self.gym_id}__a:{self.alpha}__b:{self.beta}__{self.policy_noise_2}__{self.noise_clip_2}_seed:{self.seed}"
...
if __name__ == "__main__":
    ...
    # Initialize and train behavior cloning model
    bc_actor = BehaviorCloning(action_dim=np.prod(env.action_space.shape))
    bc_state = TrainState.create(
    apply_fn=bc_actor.apply,
    params=bc_actor.init(actor_key, observation),
    tx=optax.adam(learning_rate=args.actor_lr),
    )
    ...
    def update_actor(actor_state, qf_state, batch, key):
        ...
        def actor_loss_fn(params):
            ...
            # Compute KL loss
            actor_dist = actor.apply(params, batch.observations)
            bc_dist = bc_actor.apply(bc_state.params, batch.observations)
            kl_loss = tfd.kl_divergence(actor_dist, bc_dist).mean()
            
            # get action with params
            dist = actor.apply(params, batch.observations, temperature=1.0)
            action = dist.sample(seed=key)

            # Q-value penalty (unchanged)
            q1, q2 = qf.apply(qf_state.params, batch.observations, action)
            q = jnp.minimum(q1, q2)
            alpha = args.alpha 
            lmbda = jax.lax.stop_gradient(alpha / jax.numpy.abs(q).mean())
            
            # Final actor loss
            actor_loss = (kl_loss - lmbda * q).mean()
            return actor_loss, {"actor_loss": actor_loss, "kl_loss": kl_loss}
    ...
    def update_qf(actor_state, qf_state, batch, key):
        key, next_action = get_action(key, actor_state, batch.next_observations, temperature=1.0)
        policy_noise_1 = 0.6 
        policy_noise_2 = args.policy_noise_2
        noise_clip_1 = 0.5 
        noise_clip_2 = args.noise_clip_2
    ...
    # Behavior cloning loss and training step
    
    @jax.jit
    def train_bc_step(state, batch, rng_key):
        def loss_fn(params):
            dist = bc_actor.apply(params, batch.observations, temperature=1.0)
            predicted_actions = dist.loc  # mean
            # predicted_actions = dist.sample(seed=rng_key)  # sample
            return ((predicted_actions - batch.actions) ** 2).mean()
    ...
    for step in range(args.bc_train):
        rng, rng_key = jax.random.split(key)
        batch = dataset.sample(batch_size=args.batch_size)
        bc_state, bc_loss = train_bc_step(bc_state, batch, rng_key)
        if step % 100 == 0:
            # print(f"Step {step}: BC Loss = {bc_loss:.4f}")
            writer.add_scalar("losses/bc_loss", bc_loss, step)
